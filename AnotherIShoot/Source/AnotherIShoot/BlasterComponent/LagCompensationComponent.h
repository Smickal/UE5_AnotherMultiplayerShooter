...
class ANOTHERISHOOT_API ULagCompensationComponent : public UActorComponent
{
	GENERATED_BODY()
...
public:	
	...
	FServerSideRewindResult ProjectileServerSideRewind(
		ABlasterCharacter* HitCharacter,
		const FVector_NetQuantize& TraceStart,
		const FVector_NetQuantize100& InitialProjectileVelocity,
		float HitTime
		);
...
protected:
	...
	// Shotgun
	//
	FShotgunServerSideRewindResult ShotgunConfirmedHit(const TArray<FFramePackage>& FramePackages, const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations);

	//Projectile
	FServerSideRewindResult ProjectilConfirmedHit(
